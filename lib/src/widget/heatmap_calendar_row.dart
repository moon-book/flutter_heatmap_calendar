...
class HeatMapCalendarRow extends StatelessWidget {
  ...
  HeatMapCalendarRow({
    ...
    this.onHover,
          ...
          (i) => (startDate == DateUtil.startDayOfMonth(startDate) && endDate.day - startDate.day != 7 && i < (startDate.weekday % 7)) ||
                  (endDate == DateUtil.endDayOfMonth(endDate) && endDate.day - startDate.day != 7 && i > (endDate.weekday % 7))
              ? Container(
                  width: size ?? 42,
                  height: size ?? 42,
              ...
              : HeatMapContainer(
                  ...
                  date: DateTime(startDate.year, startDate.month, startDate.day - startDate.weekday % 7 + i),
                  ...
                  selectedColor:
                      datasets?.keys.contains(DateTime(startDate.year, startDate.month, startDate.day - startDate.weekday % 7 + i)) ?? false
                          ...
                          ? colorMode == ColorMode.opacity
                              ...
                              ? colorsets?.values.first.withOpacity(
                                  (datasets?[DateTime(startDate.year, startDate.month, startDate.day + i - (startDate.weekday % 7))] ?? 1) /
                                      (maxValue ?? 1))
                              ...
                              : DatasetsUtil.getColor(
                                  colorsets, datasets?[DateTime(startDate.year, startDate.month, startDate.day + i - (startDate.weekday % 7))])
                          : null,
                ),
        ),
        super(key: key);
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: <Widget>[
        for (Widget container in dayContainers) WidgetUtil.flexibleContainer(flexible ?? false, true, container),
      ],
    );
  }
}
